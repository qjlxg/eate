...
class MarketMonitor:
    ...
    def _get_fund_data(self, fund_code):
        ...
        try:
            ...
            # --- 新增代码：增强数据校验，如果数据为空或不包含'累计净值'列，则跳过 ---
            if df.empty or '累计净值' not in df.columns:
                logger.warning("基金 %s 净值数据为空或格式不正确，跳过。", fund_code)
            ...
            # --- 修改代码：在获取最新数据前进行二次检查，确保数据有效性 ---
            if len(df) < 50: # MA50需要至少50个数据点
                logger.warning("基金 %s 数据不足50天，无法计算MA50，跳过。", fund_code)
                self.fund_data[fund_code] = None
                return
            
            ma_ratio = df['累计净值'].iloc[-1] / df['ma50'].iloc[-1] if not pd.isna(df['ma50'].iloc[-1]) and df['ma50'].iloc[-1] != 0 else np.nan
            latest_rsi = df['rsi'].iloc[-1] if not pd.isna(df['rsi'].iloc[-1]) else np.nan
